...
products.push({
  ...
  price: 4200,
  description: "Компактная клавиатура",
});
  ...
  res.type("html").send(`
    ...
    <html lang="ru">
      <head>
        ...
        <title>Практика 21</title>
      </head>
      <body>
        <main>
          <h1>Практика 21</h1>
          <p>API с авторизацией, ролями и Redis-кэшем.</p>
          <ul>
...
app.get(
  ...
  cacheMiddleware((req) => `products:all:${req.query.category || "any"}`, PRODUCTS_CACHE_TTL),
  async (req, res) => {
...
async function bootstrap() {
  await initRedis();
  app.listen(port, () => {
    console.log(`Practice 21 server started on http://localhost:${port}`);
  });
}
